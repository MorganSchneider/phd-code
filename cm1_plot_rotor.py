...
# x/y/z indices of median parcel trajectory position at t=plottime
ix = np.argmin(np.abs(xh - x_median[ti]))
iy = np.argmin(np.abs(yh - y_median[ti]))
...
# top_ax.quiver(xh[::4], zh[::3], u_xz[::3,::4], w_xz[::3,::4], color='k', scale=500, width=0.003, pivot='tail')
...
# side_ax.quiver(zh[::3], yh[::4], w_yz[::3,::4].transpose(), v_yz[::3,::4].transpose(), color='k', scale=500, width=0.003, pivot='tail')
